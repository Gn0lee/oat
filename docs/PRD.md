...
- **기술 제약** - Yahoo Finance(비공식), 환율 API 월 1,500회 무료
...
| 환율 계산 복잡 | 30분마다 환율 자동 갱신, 원화 환산 표시 |
| 리밸런싱 어려움 | 목표 비중 설정 및 현재 상태 비교 기능 |
| 수기 관리 한계 | 종목 검색 + 주가 자동 조회 |
...
| ASSET-07 | 종목 검색 | 🔴 필수 | 티커/종목명으로 검색 (한국/미국) |
| ASSET-08 | 현재가 조회 | 🔴 필수 | Yahoo Finance API 연동 |
...
| ID | 기능 | 우선순위 | 설명 |
|----|------|----------|------|
| RATE-01 | 환율 자동 갱신 | 🔴 필수 | 30분마다 캐싱 |
| RATE-02 | 주가 조회 | 🔴 필수 | 일 1회 또는 요청 시 갱신 |
| RATE-03 | 원화 환산 | 🔴 필수 | USD 자산을 KRW로 표시 |
...
| 항목 | 제약 |
|------|------|
| 주가 데이터 | Yahoo Finance (비공식, 제한적) |
| 환율 | ExchangeRate-API 무료 월 1,500회 |
