...
- **MVP 범위** - 인증(이메일/비밀번호), 가구 연결, 자산 기록(주식), 대시보드
- **자산 구조** - 자산 유형별 계층 구조 (`/assets/[type]/...`), MVP는 주식만 지원
...
### 1.2 한줄 요약
가족이 각자 보유한 다양한 자산(주식, 현금, 부동산 등)을 한곳에 모아, "우리 집 전체 자산 현황"을 실시간으로 파악하고 데이터 기반의 자산 배분 의사결정을 돕는 웹 서비스
...
| ID | 기능 | 우선순위 | 설명 |
|----|------|----------|------|
| ASSET-01 | 자산 메인 페이지 | 🔴 필수 | 자산 유형별 요약 카드, 유형 선택 진입점 |
| ASSET-02 | 전체 보유 현황 | 🔴 필수 | 모든 자산 유형 통합 목록 (정렬/필터링) |

**주식 자산 (MVP)**
| ID | 기능 | 우선순위 | 설명 |
|----|------|----------|------|
| STOCK-TX-01 | 매수 기록 | 🔴 필수 | 종목 검색 → 수량/단가/거래일 입력 |
| STOCK-TX-02 | 매도 기록 | 🔴 필수 | 보유 종목 선택 → 수량/단가/거래일 입력 |
| STOCK-TX-03 | 거래 내역 | 🔴 필수 | 매수/매도 기록 목록 조회 |
| STOCK-TX-04 | 거래 수정 | 🟡 권장 | 잘못 입력한 거래 수정 |
| STOCK-TX-05 | 거래 삭제 | 🟡 권장 | 잘못 입력한 거래 삭제 |
| STOCK-HD-01 | 주식 보유 현황 | 🔴 필수 | 현재 보유 종목 목록 (정렬/필터링) |
| STOCK-HD-02 | 종목 검색 | 🔴 필수 | 티커/종목명/초성으로 검색 (한국/미국) |
| STOCK-HD-03 | 전일 종가 조회 | 🔴 필수 | stock_master 테이블에서 조회 |
| STOCK-HD-04 | 수동 가격 입력 | 🟡 권장 | 비상장 주식용 |

**현금성 자산 (2단계)**
| ID | 기능 | 우선순위 | 설명 |
|----|------|----------|------|
| CASH-01 | 예적금 기록 | 🟢 선택 | 은행/금액/만기일 입력 |
| CASH-02 | 입출금 내역 | 🟢 선택 | 입금/출금 기록 |

**부동산/기타 (2단계)**
| ID | 기능 | 우선순위 | 설명 |
|----|------|----------|------|
| REAL-01 | 부동산 기록 | 🟢 선택 | 주소/평가액 입력 |
| OTHER-01 | 기타 자산 기록 | 🟢 선택 | 직접 입력 (자동차, 귀금속 등) |
...
| ASSET-09 | 수동 현재가 입력 | 비주식 자산(채권, 현금, 대체투자)용 현재가 직접 입력 |
| MARKET-01 | 국내 시장 동향 | KIS API 거래량순위, 등락률순위 조회 |
| MARKET-02 | 해외 시장 동향 | KIS API 해외주식 급등락, 거래량급증 조회 |
| MARKET-03 | 해외 뉴스 | KIS API 해외뉴스종합 조회 |
| ACCOUNT-01 | 계좌 관리 | 증권사 계좌 등록/수정/삭제 |
| ACCOUNT-02 | 계좌별 보유 현황 | 계좌별로 보유 종목 필터링 |
...
[자산 유형 선택 화면] (/assets)
  ...
  └─────────────────────────────────────┘
    ↓ (주식 선택 시)
[주식 거래 등록] (/assets/stock/transactions/new)
    ↓
[거래 유형 선택] → 매수 / 매도
    ↓
[종목 검색] → 티커 또는 종목명 입력
...
| 화면 | 경로 | 설명 |
|------|------|------|
| 홈 | `/home` | 총 자산 요약, 자산 유형별 카드, 빠른 액션 |

**대시보드**
| 화면 | 경로 | 설명 |
|------|------|------|
| 대시보드 메인 | `/dashboard` | 총 자산/수익률 요약, 분석 유형 선택 허브 |
| 주식 분석 | `/dashboard/stocks` | 주식 자산 상세 분석 |
| 현금 분석 | `/dashboard/cash` | 현금 자산 분석 (준비 중) |
| 부동산 분석 | `/dashboard/real-estate` | 부동산 분석 (준비 중) |
| 기타 분석 | `/dashboard/other` | 기타 자산 분석 (준비 중) |
| 비중 분석 | `/dashboard/breakdown` | 비중 분석 (탭: 소유자별/위험도별/자산군별) |

**자산**
| 화면 | 경로 | 설명 |
|------|------|------|
| 자산 메인 | `/assets` | 자산 유형별 요약 카드, 유형 선택 진입점 |
| 전체 보유 현황 | `/assets/total/holdings` | 모든 자산 통합 목록 |
| 주식 메인 | `/assets/stock` | 주식 허브 (요약, 포트폴리오 진입점, 시장 동향) |
| 주식 보유 현황 | `/assets/stock/holdings` | 주식 보유 종목 목록 |
| 주식 거래 내역 | `/assets/stock/transactions` | 매수/매도 기록 목록 |
| 주식 거래 등록 | `/assets/stock/transactions/new` | 매수/매도 기록 폼 |
| 종목 설정 | `/assets/stock/settings` | 종목별 자산유형, 위험도 관리 |
| 계좌 관리 | `/assets/stock/accounts` | 증권사 계좌 관리 (2단계) |
...
│  │                                                     │   │
│  │  수익 TOP 3              손실 TOP 3                 │   │
│  │  1. AAPL    +19.0%      1. META    -5.2%           │   │
│  │  2. NVDA    +15.5%      2. 삼성SDI  -3.1%          │   │
│  │  3. MSFT    +12.3%      3. 카카오   -2.8%          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  시장 동향 (Phase 2)                                 │   │
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │  💳 계좌 연결하기 (Phase 3)                      →  │   │
│  │  증권사 계좌를 연결하여 자산을 관리하세요 (준비 중)   │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 6.5 대시보드 메인 와이어프레임 (허브)
...
│  │  종목       | 평가금액      | 비중   | 수익률        │   │
│  │  ─────────────────────────────────────────────────  │   │
│  │  AAPL       | ₩35,000,000  | 35%   | +19.0%       │   │
│  │  TSLA       | ₩25,000,000  | 25%   | +8.5%        │   │
│  │  삼성전자    | ₩20,000,000  | 20%   | +10.0%       │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  │  소유자    | 평가금액      | 비중   | 수익률         │   │
│  │  ─────────────────────────────────────────────────  │   │
│  │  남편      | ₩69,000,000  | 55%   | +15.2%        │   │
│  │  아내      | ₩56,430,000  | 45%   | +9.8%         │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
