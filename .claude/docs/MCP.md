...
`get_financial_overview`, `get_ledger_stats` 같은 집계 응답은 공용 장부와 토큰 소유자의 개인 장부를 분리해 반환합니다. 두 장부는 합산하지 않으며, 파트너 개인 장부의 세부 내역과 합계는 포함하지 않습니다.
...
- `summary.shared`는 공용 장부만 포함합니다.
- `summary.personal`은 토큰 소유자의 개인 장부만 포함합니다.
- `summary.shared`와 `summary.personal`은 서로 합산하지 않습니다.
- 파트너 개인 장부는 상세 내역과 합계를 모두 포함하지 않습니다.
- 용돈처럼 공용 장부에서 개인에게 지급된 돈은 공용 지출로 기록되고, 개인 장부에서는 개인 수입으로 기록될 수 있습니다. 두 장부는 별도 관점입니다.
...
- 파트너 개인 장부는 상세 내역과 합계 모두 노출되지 않습니다.
- 토큰은 언제든 회수할 수 있습니다.
