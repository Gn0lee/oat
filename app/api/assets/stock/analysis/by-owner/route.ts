...
import type {
  ...
  StockOwnerAnalysisData,
  StockOwnerHoldings,
} from "@/types";

/**
 * GET /api/assets/stock/analysis/by-owner
 * 소유자별 주식 분석 데이터 조회
 */
export async function GET() {
  try {
    ...
    if (holdings.length === 0) {
      const emptyData: StockOwnerAnalysisData = {
        summary: [],
        holdings: [],
        exchangeRate,
      };
    ...
    // 소유자별 보유 종목 생성
    const ownerHoldings: StockOwnerHoldings[] = Array.from(
      ownerMap.entries(),
    ).map(([ownerId, { name, stocks }]) => ({
      ownerId,
      ownerName: name,
      stocks,
    }));
    ...
    const result: StockOwnerAnalysisData = {
      summary,
